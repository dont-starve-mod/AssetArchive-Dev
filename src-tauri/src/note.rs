...
/* 
    ...
    lua.context(|lua_ctx| {
        ...
        globals.set("-1290-", true)?; // 似乎只是表索引操作, 不会检查全局变量名的合法性
        globals.set(1, -1)?; // 也可以用数字作为索引 XD
    ...
    lua.context(|lua_ctx| {
        ...
        dbg!(globals.get::<_, String>("int_var")?); // "42"
        // dbg!(globals.get::<_, i64>("string_var")?); // 报错! 无法转换为数字
        globals.set("string_var", "42")?; // 给一个字符串
        dbg!(globals.get::<_, i64>("string_var")?); 
    ...
    lua.context(|lua_ctx| {
        ...
        assert_eq!(map_table.len()?, 0); // 哈希表的len是0
        ...
        impl UserData for Vec2 {
            fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
                ...
                methods.add_meta_function(MetaMethod::ToString, |_, (vec): (Vec2)| {
                    Ok(format!("vec2({}, {})", vec.0, vec.1))
                })
            }
        }
        ...
        assert!(
            (lua_ctx
                .load("print(vec2(114,514)); return (vec2(1, 2) + vec2(2, 2)):magnitude()")
                .eval::<f32>()?
                - 5.0)
                .abs()
                < f32::EPSILON
        ...
        // lua_ctx.load("sketchy()").exec()?; // 变量失效, 报错
