-- https://github.com/Phrogz/SLAXML/blob/master/slaxdom.lua
-- Optional parser that creates a flat DOM from parsing
local SLAXML = require 'slaxml'

-- useful methods for xml dom
local dom_methods = { }

function dom_methods:find_elements(name)
    local result = {}
    for _,v in ipairs(self.kids)do
        if v.type == "element" and v.name == name then
            table.insert(result, setmetatable(v, {__index = dom_methods}))
        end
    end
    return result
end

function dom_methods:find_elements_recursive(name)
    local result = dom_methods.find_elements(self, name)
    for _,v in ipairs(self.kids)do
        table.extend(result, dom_methods.find_elements_recursive(v, name))
    end
    return result
end
...
function SLAXML:dom(xml,opts)
    ...
    return setmetatable(doc, {__index = dom_methods}) -- change: add methods
end
