...
QuickLook = Class(function(self, filepath)
	self.filepath = filepath
	self.data = {}
	self.ext = ExtOf(filepath)

	local f = CreateReader(filepath)
	if f == nil then
		table.insert(self.data, { type = "error", error = "Cannot open file: "..filepath })
		return
	end
	...
	local prefer_txt = table.contains({"txt", "lua", "json", "xml", "csv", "html", "htm", "md", "ps", "vs", "scml"}, self.ext)
	...
	elseif perfer_bin then
		...
		local size = GetFileSize(self.filepath)
		local mtime = GetMTime(self.filepath)
...
function QuickLook:LoadAsXml(f, error)
	...
	if xml.error then
		error(xml.error)
		table.insert(self.data, { type = "xml", error = xml.error })
	else
		local texname = xml.texname
		local imgs = xml.imgs
		table.insert(self.data, { type = "xml", texname = texname, imgs = imgs })
		-- try to load tex
		local inst2 = self:TryToOpen(xml.texname)
		if inst2 == nil then
			table.insert(self.data, { type = "ref:tex", exists = false, name = texname })
		else
			table.insert(self.data, { type = "ref:tex", exists = true, name = texname, path = inst2:GetPath(),
...
function QuickLook:LoadAsTex(f, error)
	...
	if tex.error then
		error(tex.error)
		table.insert(self.data, { type = "tex", error = tex.error, format = format })
	else
		local w, h = tex:GetSize()
		local bytes = tex:GetImageBytes(0)
		bytes = Image.ToRGBA(bytes, w, h)
		table.insert(self.data, { type = "tex", img_json = Image.EncodeJson(bytes, w, h), format = format })
	end
end
...
function is_utf8_lossy(str)
	...
	local check, last_index = is_utf8(str)
...
IpcHandlers.Register("quicklook_load", function(param)
	...
	local look = QuickLook(filepath)
	return json.encode_compliant(look.data)
end)
